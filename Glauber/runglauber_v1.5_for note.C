...
//
// To run the macro: 
///tuos@[~/Desktop/DesktopArchive/temp/forStudent]root -l 
///root [0] .L runglauber_v1.5.C++ 
///root [1] runAndSaveNtuple(10000,"Pb","Pb",64,0.4,"Phob_Glau_PbPb_test_v15.root")
///Setting up nucleus Pb
///Setting up nucleus Pb
///Event # 9950 x-sect = 7.65756 +- 0.0767639 b        
///Done!
///root [2] 
///root [2] .q
//
...
//     Ecc2PART = v[22] = participant eccentricity, but calculated with formalism for the triangularity as detailed in 
Phys.Rev.C81:054905(2010) Equation 4
...
//
//------------
// Version 1.3:  modification by djh to include two definitions of the transverse overlap area of two nuclei.  Added 
the following
//     SPART = v[19] = overlap area that is relevant if one uses the "participant" eccentricity EccPART.  See: 
PRC77, 014906 (2008)
//     SPART4 = v[20] = overlap area above multiplied by a factor of 4 to better match Valdimir's result (and 
because we think the factor of 4 better matches the actual 
geometrical overlap)
//
//     S12RP_Npart = v[21] = from Vladimir Korotkikh. Eur.Phys.J.C66:173,2010; arXiv:0910.3029 [hep-ph]; 
Phys.Rev.C76 (2007) 024905: but here it is the overlap area 
using participants (not collisions!) and calculated relative to the original reaction plane of the nuclei (i.e. not 
modified event-by-event to match the maximal axis of the 
...
//              THIS is the original definition of the eccentricity from participating nucleons - typically not used any more 
in physics results for flow.
//
//      EccPART = v[18] = TMath::Sqrt((fSy2-fSx2)*(fSy2-fSx2)+4.*fSxy*fSxy)/(fSy2+fSx2);  //v1.2  
//              THIS is the "participant" eccentricity, calculated in a plane that is rotated event-by-event to maximize the 
eccentricity as defined by the participants.  This 
variable seems relevant as it unifies Cu+Cu and Pb+Pb flow data from RHIC.  See: PRC77, 014906 (2008)
//
...
class TGlauNucleon : public TNamed
{
   ...
   public:
      TGlauNucleon() : fX(0), fY(0), fZ(0), fInNucleusA(0), fNColl(0) {} //Constructor initialization list, TGlauNucleon() 
	  {fX = fY= fZ = flnNucleusA = fNColl = 0;}
...
void TGlauberMC::Run(Int_t nevents)
{
   ...
   if (fnt == 0) {
      fnt = new TNtuple(name,title,
			//			
			"Npart:Ncoll:B:MeanX:MeanY:MeanX2:MeanY2:MeanXY:VarX:VarY:VarXY:MeanXSystem:MeanYSyste
			m:MeanXA:MeanYA:MeanXB:MeanYB");
			//			
			"Npart:Ncoll:B:MeanX:MeanY:MeanX2:MeanY2:MeanXY:VarX:VarY:VarXY:MeanXSystem:MeanYSyste
			m:MeanXA:MeanYA:MeanXB:MeanYB:EccRP:EccPAR
			T"); //v1.2
			//			
			"Npart:Ncoll:B:MeanX:MeanY:MeanX2:MeanY2:MeanXY:VarX:VarY:VarXY:MeanXSystem:MeanYSyste
			m:MeanXA:MeanYA:MeanXB:MeanYB:EccRP:EccPAR
			T:SPART:SPART4:S12RP_Npart"); //v1.3
			//			
			"Npart:Ncoll:B:MeanX:MeanY:MeanX2:MeanY2:MeanXY:VarX:VarY:VarXY:MeanXSystem:MeanYSyste
			m:MeanXA:MeanYA:MeanXB:MeanYB:EccRP:EccPAR
			T:SPART:SPART4:S12RP_Npart:Ecc2PART:Psi2PART:Ecc3PART:Psi3PART"); //v1.4
			"Npart:Ncoll:B:MeanX:MeanY:MeanX2:MeanY2:MeanXY:VarX:VarY:VarXY:MeanXSystem:MeanYSyste
			m:MeanXA:MeanYA:MeanXB:MeanYB:EccRP:EccPAR
			T:SPART:SPART4:S12RP_Npart:Ecc2PART:Psi2PART:Ecc3PART:Psi3PART:Ecc1PART:Psi1PART:
			Ecc4PART:Psi4PART:Ecc5PART:Psi5PART:Ecc6PA
			RT:Psi6PART"); //v1.5
      fnt->SetDirectory(0);
   }
   ...
   for (int i = 0;i<nevents;i++) {
      ...
      v[18] = TMath::Sqrt((fSy2-fSx2)*(fSy2-fSx2)+4.*fSxy*fSxy)/(fSy2+fSx2);  //v1.2  Eqn A6 of PRC77, 014906 
	  (2008)
	...
	v[19] = TMath::Pi()*TMath::Sqrt(v[19]); //v1.3  Eqn A8 of PRC77, 014906 (2008)
      ...
      v[21] = 4.0*TMath::Pi()*TMath::Sqrt(fMeanX2)*TMath::Sqrt(fMeanY2); //v1.3  Eur.Phys.J.C66:173,2010 but 
	  with participants and relative to original Reaction Plane
      ...
      if (!(i%50)) cout << "Event # " << i << " x-sect = " << GetTotXSect() << " +- " << GetTotXSectErr() << " b        
	  \r" << flush;  
   }
   cout << endl << "Done!" << endl;
}
