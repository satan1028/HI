...
//------------------------Deal with the number and get the output stored in txt files and root files
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		if(isSum) f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/Bessfun/pPbDataV205m30/AnaV_Sum_%d.root",ifile));
		else f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/Bessfun/pPbDataV205m30/AnaV_Prod_%d.root",ifile));
