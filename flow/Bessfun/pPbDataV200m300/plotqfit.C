...
ffit->SetParameters(0.2,0.05,avgmult[xbin]);
ffit->FixParameter(2,avgmult[xbin]);
f1fit->SetParNames("g2","v2","M");
f1fit->SetParameters(0.2,0.05,avgmult[xbin]);
...
ffit0->SetParameters(ffit->GetParameter(0),0,ffit->GetParameter(2));
...
hqx_cp->Scale(1./hqx_cp->Integral(0,-1,"width"));
hqx_cp->GetYaxis()->SetTitle("#frac{dN}{dq_{x}}");
hqx_cp->GetYaxis()->SetRangeUser(0,1);
hqx_cp->Fit(Form("f1fit"),"R","",0,10);
f1fit0->SetParameters(f1fit->GetParameter(0),0,f1fit->GetParameter(2));
...
hqy->GetYaxis()->SetRangeUser(1e-10,1);
...
hqy_cp->Scale(1./hqy_cp->Integral(0,-1,"width"));
hqy_cp->GetYaxis()->SetTitle("#frac{dN}{dq_{y}}");
hqy_cp->GetYaxis()->SetRangeUser(0,1);
