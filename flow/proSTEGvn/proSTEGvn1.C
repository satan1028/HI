...
void proSTEGvn1()
{
  ...
  TFile f(Form("/tmp/xuq7/pPbDataV201m%d/vndata_50k_%d.root",mult,ifile), "RECREATE","ROOT file with histograms & tree");
...
//  tree->Branch("phirg", &b_phirg, "phirg/F");  // RP angle;
 ...
 //	V2vsPt->SetParameters(4.81159,1.80783,3.69272,3.11889,0.931485);	//Real data V~0.05
  ...
  TF1 *V2vsPt = new TF1("V2vsPt","0.2*((x/3.31699)^2.35142/(1+(x/3.49188)^3.54429))*(.00005+(1/x)^1.50600)",0.3,6.0);
