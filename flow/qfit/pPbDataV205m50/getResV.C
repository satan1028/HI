...
//------------------------Deal with the number and get the output stored in txt files and root files
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/qfit/pPbDataV205m50/AnaV_Sum_%d.root",ifile));
		...
		for(int ibin=0;ibin<nbin;ibin++){
                                ...
                                hq2[ibin]->Add(hq2_t);
                                TH1D* hvq2_t = (TH1D*)f[ifile]->Get(Form("hvq2_%d",ibin));
                                hvq2[ibin]->Add(hvq2_t);
	...
	for(int ibin=0;ibin<nbin;ibin++){
                ...
                hqx[ibin]->Write("hqx"); hqy[ibin]->Write("hqy");hq2[ibin]->Write("hq2");hvq2[ibin]->Write("hvq2");hq2nonf[ibin]->Write("hq2nonf");
