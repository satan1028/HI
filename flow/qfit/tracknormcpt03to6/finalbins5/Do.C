...
void dofirst(){
	...
	for(int i=start;i<end;i++){
		name=Form("/scratch/xuq7/flow/pbsjoboutput/qfit/tracknormcpt03to6/finalbins5/%s/AnaV_Sum_%d.root",dir.c_str(),i);
		remove(name.Data());
...
void dosecondsub(){
	...
	TFile *inf;
   	infname="mergedV_Sum_sub.root";
        ...
        for(int i=start;i<end;i++){
                name=Form("/scratch/xuq7/flow/pbsjoboutput/qfit/tracknormcpt03to6/finalbins5/%s/Anav_Sum_sub_%d.root",dir.c_str(),i);
                remove(name.Data());
                qfit *l = new qfit(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(1);
...
void dothird(){
        ...
        TString name;
        for(int i=start;i<end;i++){
                name=Form("/scratch/xuq7/flow/pbsjoboutput/qfit/tracknormcpt03to6/finalbins5/coarsebins/%s/Anaveta_Sum_%d.root",dir.c_str(),i);
                remove(name.Data());
                qfit *l = new qfit(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(0);
