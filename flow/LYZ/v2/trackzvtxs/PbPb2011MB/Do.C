...
void dofirst(){
	...
	for(int i=start;i<end;i++){
		if(SumorProd=="Sum")	name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/trackzvtxs/AnaV_Sum_%d.root",i);
		else	name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/trackzvtxs/AnaV_Prod_%d.root",i);
		remove(name.Data());
...
void dosecond(){
	string SumorProd=getenv("SUMORPROD");
      ...
      //  for(int i=start;i<end;i++){
            for(int j=0;j<9;j++){
                i=temp[j];
                if(SumorProd=="Sum")       name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/trackzvtxs/Anav_Prod_%d.root",i);
                else            name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/trackzvtxs/Anav_Prod2_%d.root",i);
                remove(name.Data());
                LYZ *l = new LYZ(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(1);
...
void dosecondsub(){
	...
	TFile *inf;
   	if(SumorProd=="Sum")       infname="mergedV_Sum_sub.root";
        ...
        for(int i=start;i<end;i++){
                if(SumorProd=="Sum")       name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/trackzvtxs/Anav_Prod_sub_%d.root",i);
                else            name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/trackzvtxs/Anav_Prod2_sub_%d.root",i);
                remove(name.Data());
                LYZ *l = new LYZ(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(1);
...
void dothird(){
        ...
        TString name;
        for(int i=start;i<end;i++){
                if(SumorProd=="Sum")       name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/finereta/trackzvtxs/Anaveta_Prod_%d.root",i);
                else            name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/finereta/trackzvtxs/Anaveta_Prod2_%d.root",i);
                remove(name.Data());
                LYZ *l = new LYZ(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(0);
