...
for(int i=0;i<ntotbin;i++){
	...
	TGraphErrors *grProd_u=plotTG(i,0,1,dirname,20,2);
	TGraphErrors *grProd=plotTG(i,0,0,dirname,24,4);
	...
	tl->AddEntry(grProd_u,"LYZ use weight","lp");
	tl->AddEntry(grProd,"LYZ weight=1","lp");
	if(i==0 || i==3) 
...
TGraphErrors* plotTG(int i,bool isSum, bool isUse, TString dirname, int marker, int color){
	if(!isUse){
		if(!isSum)
