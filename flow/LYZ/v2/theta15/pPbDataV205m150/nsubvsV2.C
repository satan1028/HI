...
//------------------------Deal with the number and get the output stored in txt files and root files
	...
	for(int ifile=start; ifile<end; ifile++){
		if(isSum) f[ifile] = TFile::Open(Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/pPbDataV205m150/AnaV_Sum_%d.root",ifile));
		else f[ifile] = TFile::Open(Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/pPbDataV205m150/AnaV_Prod_%d.root",ifile));
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		Vmeanmean[ibin]+=Vmean[ibin][0]/nsamples;
		Vmeanmean[ibin+nbin]+=TMath::Power(Vmean[ibin]/nsamples,2);
		chi_[ibin].Write("chi");
