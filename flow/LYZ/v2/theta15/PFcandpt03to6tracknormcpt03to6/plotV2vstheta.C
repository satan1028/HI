...
void plotV2vstheta(){
 ...
 //     tl->SetBorderStyle(0);
        ...
        tl->DrawLatex(1,0.05,Form("Multiplicity %d to %d",trkpointmin[trkbin],trkpointmax[trkbin]));
        l->Draw("same");
        lup->Draw("same");
        ldown->Draw("same");
	if(isSum==0)c1->SaveAs("hV2theta_Prod.png");
	else c1->SaveAs(Form("M%d%d/hV2theta_Sum.png",trkpointmax[trkbin],trkpointmin[trkbin]));
}
