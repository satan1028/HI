...
void getResv(){
	...
	for(int ibin=0;ibin<nbin;ibin++){
        ...
        for(int ifile=0; ifile<nFileAlltrack; ifile++){
	        if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/theta15/PFcandpt03to6tracknormcpt03to6/%s/Anav_Prod_%d.root",mdir.c_str(),ifile));
	        else f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/theta15/PFcandpt03to6tracknormcpt03to6/%s/Anav_Prod2_%d.root",mdir.c_str(),ifile));
