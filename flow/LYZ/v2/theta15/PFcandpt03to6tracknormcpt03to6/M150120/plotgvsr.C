...
const int plotnu[nnu]={9,10,11};
//const int plotnu[nnu]={0,1,2,3};
void plotgvsr(){
    gStyle->SetOptStat(kFALSE);
TCanvas *c1 = new TCanvas();
c1->SetLogy();
TH1D* hFrame = new TH1D("","",1000,0,1);
hFrame->GetYaxis()->SetRangeUser(5e-8,1e-1);
hFrame->GetXaxis()->SetRangeUser(0.12,0.30);
...
t->SetTextSize(0.03);
t->SetTextFont(42);
t->DrawLatex(0.5,0.2,Form("PF candidate, %d < mult <%d, %.1f < p_{T} < %.1f", trkbin[xbin+1],trkbin[xbin],ptmin,ptmax));
...
c1->Print("gvsr_thetas_other.png");
