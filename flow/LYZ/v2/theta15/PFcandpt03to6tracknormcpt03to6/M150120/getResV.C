...
//------------------------Deal with the number and get the output stored in txt files and root files
	...
	for(int ifile=0; ifile<nFileAllPF; ifile++){
		if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/theta15/PFcandpt03to6tracknormcpt03to6/%s/AnaV_Sum_%d.root",dir.c_str(),ifile));
		else f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/theta15/PFcandpt03to6tracknormcpt03to6/%s/AnaV_Prod_%d.root",dir.c_str(),ifile));
		...
		for(int iptbin=0;iptbin<nptV;iptbin++){
			...
			sigma2_[ibin][iptbin]/=ntheta;
				...
				fstrV<<ibin<<"\t"<<iptbin<<"\t"<<itheta<<"\t"<<r0[ibin][iptbin][itheta]<<"\t"<<V[ibin][iptbin][itheta]<<"\t"<<sigma2[ibin][iptbin][itheta]<<"\t"<<chi[ibin][iptbin][itheta]<<"\t"<<deltaV[ibin][iptbin][itheta]<<endl;
                                if(itheta==ntheta-1) fstrV<<"\t\t\t\t\t"<<sigma2_[ibin][iptbin]<<"\t"<<chi_[ibin][iptbin]<<endl;
			}
			fstrV<<endl;
