...
void getResvsub(){
	...
	ofstream fstrvns;       TFile *f_;
        if(isSum==0){
                fstrvns.open("nsv2_Prod.txt");
                f_= new TFile("v2_nsvssub_Prod.root","Recreate");
        }
        else {
                fstrvns.open("nsv2_Sum.txt");
                f_= new TFile("v2_nsvssub_Sum.root","Recreate");
        }
	double theta[ntheta];
	TFile *f[nFileAll];
	...
	TVectorD vmeanmean[nbin],deltavmeanmean[nbin];
	...
	for(int ibin=0;ibin<nbin;ibin++){
        ...
        for(int ifile=start; ifile<end; ifile++){
	        if(isSum) f[ifile] = TFile::Open(Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/pPbDataV205m150/Anav_Prod_sub_%d.root",ifile));
	        else f[ifile] = TFile::Open(Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/pPbDataV205m150/Anav_Prod2_sub_%d.root",ifile));
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		deltavmeanmean[ibin][iptbin]+=TMath::Power(vmean[ibin][iptbin]/nsamples,2);
		}
	...
	f_->cd();
	for(ibin=0;ibin<nbin;ibin++){
	TDirectory *dir_ = f_->mkdir(Form("D_%d",ibin));
	dir_->cd();
		for(int iptbin=0;iptbin<nptv; iptbin++){
			deltavmeanmean[ibin][iptbin]=TMath::Sqrt(deltavmeanmean[ibin][iptbin]*nsamples-vmeanmean[ibin][iptbin]*vmeanmean[ibin][iptbin]);
			fstrvns<<vmeanmean[ibin][iptbin]<<"\t"<<deltavmeanmean[ibin][iptbin]<<endl;
		}
		avgpt[ibin].Write("avgpt");	vmeanmean[ibin].Write("vmeanmean");deltavmeanmean[ibin].Write("deltavmeanmean");
	}
	f_->Close();
	infile->Close();
	fout->Close();
