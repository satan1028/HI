...
TGraphErrors *gr10 = plot("tracknormcpt03to6",0,2,20,0);
...
leg->AddEntry(grpf2,Form("PF candidate,%.1f < p_{T} < %.1f (GeV/c)",0.3,6.0),"P");
...
double  V2_0[ntotbin], V2err_0[ntotbin];
...
TVectorD *vecV2_0=(TVectorD*)mergedV->Get(Form("D_%d/Vmean",ibin));
TVectorD *vecV2err_0=(TVectorD*)mergedV->Get(Form("D_%d/deltaVmean",ibin));
Ntrk[i]+=(*vecNtrk)[ibin];
Mult[i]+=(*vecMult)[ibin];
Nevent[i]+=(*vecNevent)[ibin];
V2_0[i]+=(*vecV2_0)[xpt];
V2err_0[i]+=(*vecV2err_0)[xpt];
}
Ntrk[i]/=Nevent[i]; 	Mult[i]/=Nevent[i];    
V2_0[i]/=nbin[i];	V2err_0[i]/=nbin[i];
}
else{
...
TVectorD *vecNevent = (TVectorD*)mergedV->Get("Nevent");
TVectorD *vecV2_0=(TVectorD*)mergedV->Get(Form("D_%d/Vmean",xbin));
TVectorD *vecV2err_0=(TVectorD*)mergedV->Get(Form("D_%d/deltaVmean",xbin));
Ntrk[i]=(*vecNtrk)[xbin];
...
fout<<"\t"<<V2err_0[i]<<"\t"<<V2err_1[i]<<"\t"<<V2err_2[i]<<"\t"<<V2err_3[i]<<endl;
}
...
if(debug==0)TGraphErrors *gr=new TGraphErrors(ntotbin,Ntrk,V2_0,0,V2err_0);
