...
void dosecond(){
	...
	for(int i=start;i<end;i++){	
		if(isSum)	name=Form("/scratch/xuq7/flow/pbsjoboutput/pPbDataV205m100/Anav_Prod_%d.root",i);
		else		name=Form("/scratch/xuq7/flow/pbsjoboutput/pPbDataV205m100/Anav_Prod2_%d.root",i);
		remove(name.Data());
