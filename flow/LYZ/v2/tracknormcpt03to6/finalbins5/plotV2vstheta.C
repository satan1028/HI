...
void plotV2vstheta(){
	...
	TVectorD* vecVmean = (TVectorD*)f->Get(Form("Vmean"));
        TVectorD* vecV = (TVectorD*)f->Get(Form("D_%d/V",ibin));
        TVectorD* vecdeltaV = (TVectorD*)f->Get(Form("D_%d/deltaV",ibin));

        double Vmean = (*vecVmean)[ibin];
 ...
 //     tl->SetBorderStyle(0);
        ...
        tl->SetNDC();
        tl->DrawLatex(0.5,0.85,Form("Multiplicity %d to %d",trkpointmin[trkbin]+5*ibin,trkpointmin[trkbin]+5*ibin+5));
        l->Draw("same");
        lup->Draw("same");
        ldown->Draw("same");
        }
