...
void dofirst(){
	...
	for(int i=start;i<end;i++){
		if(SumorProd=="Sum")	name=Form("/scratch/xuq7/flow/pbsjoboutput/etatracknormcpt03to6/%s/AnaV_Sum_%d.root",dir.c_str(),i);
		else	name=Form("/scratch/xuq7/flow/pbsjoboutput/etatracknormcpt03to6/%s/AnaV_Prod_%d.root",dir.c_str(),i);
		remove(name.Data());
...
void dosecond(){
	...
	string dir=getenv("DIR");
        TString name;
        for(int i=start;i<end;i++){
                if(SumorProd=="Sum")       name=Form("/scratch/xuq7/flow/pbsjoboutput/etatracknormcpt03to6/%s/Anav_Prod_%d.root",dir.c_str(),i);
                else            name=Form("/scratch/xuq7/flow/pbsjoboutput/etatracknormcpt03to6/%s/Anav_Prod2_%d.root",dir.c_str(),i);
                remove(name.Data());
                LYZ *l = new LYZ(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(1);
...
void dosecondsub(){
	...
	TFile *inf;
   	if(SumorProd=="Sum")       infname="mergedV_Sum_sub.root";
        ...
        for(int i=start;i<end;i++){
                if(SumorProd=="Sum")       name=Form("/scratch/xuq7/flow/pbsjoboutput/etatracknormcpt03to6/%s/Anav_Prod_sub_%d.root",dir.c_str(),i);
                else            name=Form("/scratch/xuq7/flow/pbsjoboutput/etatracknormcpt03to6/%s/Anav_Prod2_sub_%d.root",dir.c_str(),i);
                remove(name.Data());
                LYZ *l = new LYZ(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(1);
