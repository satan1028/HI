...
//16 bins - hiSelectedTracks
  ...
  double minptcut[NB_SLICE_PT] = {0.3, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0};
  double maxptcut[NB_SLICE_PT] = {0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 6.0};
  ...
  outR.close();
     ...
     char *file1 = "/lio/lfs/cms/store/user/qixu/flow/PACorrHM/skim/tracknormcpt03to6/multiM150120/FlowLYZHMPbp/skimTreeTrack_100_1_L3v.root";
     TFile *f1 = new TFile(file1);
     TTree *t1 = (TTree*)f1->Get("demo/TrackTree");
     t1->SetBranchAddress("mult", &rmult);
	...
	if(ntrk>=150 || ntrk <120)continue;
...

	 {
	   //if(eta0[nh]<-0.35||eta0[nh]>0.35) continue;
           ...
           if(pt0[nh]<0.3||pt0[nh]>6.0) continue;
           hptt->Fill(pt0[nh]);
           hphit->Fill(phi0[nh]);
           hetat->Fill(eta0[nh]);
  ...
  for(k1=0; k1<kt; k1++) 
    ...
    cout<<"sigma2 = "<<sigma2<<" ,  V2mean/sqrt(sigma2) = "<<chi2<<" ,  V2mean/sqrt(rmultmean) ="<<V2mean/sqrt(rmultmean)<<endl;


    outG.open("G2mod2.dat");
      for(k2=0; k2<kr; k2++){                                     
	outG<<G2mod2[0][k2]<<endl;
      }
    outG.close();
...
//          //dlng02_dw[k1][islc][islcpt]+=(cos(2.0*2.*phi0[nh]-theta[k1])*(cos(r02[k1]*Qtheta[k1])+i*sin(r02[k1]*Qtheta[k1])));
          temp2=1.*cos(2.*phi0[nh]-theta[k1]);
          ...
          dlng02_dw[k1][islc][islcpt]+=(cos(1.0*(2.*phi0[nh]-theta[k1]))/(1.+i*r02[k1]*temp2));
          dlng024_dw[k1][islc][islcpt]+=(cos(2.0*(2.*phi0[nh]-theta[k1]))/(1.+i*r02[k1]*temp2));
          dlng026_dw[k1][islc][islcpt]+=(cos(3.0*(2.*phi0[nh]-theta[k1]))/(1.+i*r02[k1]*temp2));
          dlng028_dw[k1][islc][islcpt]+=(cos(4.0*(2.*phi0[nh]-theta[k1]))/(1.+i*r02[k1]*temp2));
