...
LYZ::calcV(int way)	//way=0: Prod way=1: Sum
{
   using namespace std;
	...
	double Q[nptV][ntheta];	double Qx[nptV];	double Qy[nptV];	double tempG[nptV][ntheta];
	int mult;
		...
		for(int iptbin=0; iptbin<nptV;iptbin++){
			...
			for(int itheta=0;itheta<ntheta;itheta++){
				...
				for(int ir=0; ir<nstepr; ir++){
					if(way!=0)
					g[iptbin][itheta][ir]=TComplex::Exp(TComplex(0,r[xbin][ir]*Q[iptbin][itheta]));//g[itheta][0]
					//g[iptbin][itheta][ir]=TComplex::Exp(TComplex(0,r[xbin][ir]*tempG[iptbin][itheta]));
