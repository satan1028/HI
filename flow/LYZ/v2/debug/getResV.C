...
//------------------------Deal with the number and get the output stored in txt files and root files
	...
	for(int ifile=0; ifile<nFileAll; ifile++){
		if(isSum) f[ifile] = TFile::Open(Form("/home/xuq7/HI/flow/LYZ/v2/debug/AnaV_Sum_%d.root",ifile));
		else f[ifile] = TFile::Open(Form("/home/xuq7/HI/flow/LYZ/v2/debug/AnaV_Prod_%d.root",ifile));
