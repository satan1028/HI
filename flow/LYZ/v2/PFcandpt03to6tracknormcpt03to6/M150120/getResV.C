...
//------------------------Deal with the number and get the output stored in txt files and root files
	...
	for(int ifile=0; ifile<nFileAllPF; ifile++){
		if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/PFcandpt03to6tracknormcpt03to6/%s/AnaV_Sum_%d.root",dir.c_str(),ifile));
		else f[ifile] = TFile::Open(Form("/scratch/xuq7/flow/pbsjoboutput/PFcandpt03to6tracknormcpt03to6/%s/AnaV_Prod_%d.root",dir.c_str(),ifile));
