...
const int marker[ntheta] = {27,32,24,30,31};
const int color[ntheta] = {1,1,2,2,4};
int ibin=0;
void plotv2vspt_theta(){
...
makeMultiPanelCanvas(c1,3,2,0,0,0.25,0.2,0.03);
    ...
    TGraphErrors* grProd_theta[ntheta];
for(int i=0;i<ntotbin;i++){
	c1->cd(i+1);
	if(i!=ntotbin-1)
	TGraphErrors *gr24=new TGraphErrors(npt24,pt,v24[i],0,v24err[i]);
        for(int itheta=0;itheta<ntheta;itheta++)
	grProd_theta[itheta]=plotTG(i,0,itheta,marker[itheta],color[itheta]);
	...
	gr24->Draw("Psame");
        for(int itheta=0;itheta<ntheta;itheta++)
	grProd_theta[itheta]->Draw("Psame");
	grProd->Draw("Psame");
	...
	tl->AddEntry(gr24,"v2 4-particle cum","lp");
        for(int itheta=0;itheta<ntheta;itheta++)
	tl->AddEntry(grProd_theta[itheta],Form("LYZ i_{#theta} = %d",itheta),"lp");
	...
	c1->cd(ntotbin+1);
	TLatex *tlx0 = new TLatex(0.12,0.3,Form("%s","PFcandidate"));
	...
	tlx1->Draw("same");
	tl->Draw("same");
	c1->Print(Form("v2vspt_difftheta.png"));

}

TGraphErrors* plotTG(int i,bool isSum, int itheta, int marker, int color){
	        if(!isSum)
                        TFile *f = TFile::Open(Form("M%d%d/mergedv_Prod2.root",trkpointmax[i],trkpointmin[i]));
                else
                        TFile *f = TFile::Open(Form("M%d%d/mergedv_Prod.root",trkpointmax[i],trkpointmin[i]));
        if(itheta==-1){
        TVectorD *vecDv2 = (TVectorD*)f->Get(Form("D_%d/vmean",ibin));
        TVectorD *vecDv2err = (TVectorD*)f->Get(Form("D_%d/deltavmean",ibin));
        }
        else{
        TVectorD *vecDv2 = (TVectorD*)f->Get(Form("D_%d/D_%d/v",ibin,itheta));
        TVectorD *vecDv2err = (TVectorD*)f->Get(Form("D_%d/D_%d/deltav",ibin,itheta));
        }
