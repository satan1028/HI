...
LYZ::calcV(int way)	//way=0: Prod way=1: Sum
{
   using namespace std;
	...
	int mult, ntrk;
        ...
        TTree* t = (TTree*)infile->Get("demo/PFTree");
        t->SetBranchAddress("nPFpart",&mult);
        t->SetBranchAddress("ntrk",&ntrk);
        t->SetBranchAddress("pfpt",pt);
        t->SetBranchAddress("pfphi",phi);
        t->SetBranchAddress("pfeta",eta);
        t->SetBranchAddress("pfid",id);
	...
	//	if(ievt%10000==0) cout<<"has processed "<<ievt<<" events"<<endl;
		...
		for(int imult=0;imult<mult;imult++){
			if(eta[imult]<-5.0||eta[imult]>5.0) continue;
                        if(pt[imult]<pfptmin||pt[imult]>pfptmax) continue; //event selection
			//if(id[imult]>5 || id[imult]<1) continue;
			if(id[imult] != 1) continue;	// only charged hardons
...
void LYZ::calcv(TString res, int way, int isample){	//way=0: product way=1: sum
	...
	for(int ibin=0;ibin<nbin;ibin++){
		if(isample>=0)
                	r0res[ibin] = (TVectorD*)fres->Get(Form("s_%d/D_%d/r0",ibin,isample));
                else
