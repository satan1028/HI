void plotnsvsV2sub(){
	...
	TVectorD* vecsigmaVmeanmean = (TVectorD*)f->Get(Form("D_%d/sigmaVmeanmean",xbin));
	...
	double *sigmaVmeanmean = vecsigmaVmeanmean->GetMatrixArray();
	TGraphErrors* gr = new TGraphErrors(N,nsamples,Vmeanmean,0,sigmaVmeanmean);
	gr->SetTitle("");
