...
void getResv(){
	...
	V_mean.ResizeTo(nbin);deltaV_mean.ResizeTo(nbin);
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		V_mean[ibin] = (TVectorD*) infile->Get(Form("D_%d/Vmean",ibin));
		deltaV_mean[ibin] = (TVectorD*) infile->Get(Form("D_%d/deltaVmean",ibin));
		for(int itheta=0;itheta<ntheta;itheta++){
        ...
        for(int ifile=0; ifile<nFileAll; ifile++){
	        if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/tracknormcpt03to6/finalbins5/%s/Anav_Prod_%d.root",mdir.c_str(),ifile));
	        else f[ifile] = TFile::Open(Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/tracknormcpt03to6/finalbins5/%s/Anav_Prod2_%d.root",mdir.c_str(),ifile));
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		avgmultall[ibin]=1.0*totmultall[ibin]/Nevent[ibin];
                ...
                hptre[ibin] = (TH1D*)hpt[ibin]->Rebin(NbinY,"hptre",ptbinhisto);
                hpteffcorr[ibin] = (TH1D*)hptre[ibin]->Clone(Form("hpteffcorr_%d",ibin));
		...
		V_intcorr[ibin]/=hpteffcorr[ibin]->Integral(hpteffcorr[ibin]->GetXaxis()->FindBin(ptbinv[0]),hpteffcorr[ibin]->GetXaxis()->FindBin(ptbinv[nptv]));
		V_intcorrerr[ibin]/=hpteffcorr[ibin]->Integral(hpteffcorr[ibin]->GetXaxis()->FindBin(ptbinv[0]),hpteffcorr[ibin]->GetXaxis()->FindBin(ptbinv[nptv]));
		delete hptre[ibin];
	}
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		fstrv<<"Integral\t\t"<<totmultall[ibin]<<"\t"<<hpt[ibin]->Integral(hpt[ibin]->GetXaxis()->FindBin(ptbinv[0]),hpt[ibin]->GetXaxis()->FindBin(ptbinv[nptv]))<<"\t"<<hpteffcorr[ibin]->Integral(hpteffcorr[ibin]->GetXaxis()->FindBin(ptbinv[0]),hpteffcorr[ibin]->GetXaxis()->FindBin(ptbinv[nptv]))<<endl;
