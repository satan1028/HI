...
void dofirst(){
	...
	for(int i=start;i<end;i++){
		if(SumorProd=="Sum")	name=Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/PFcandpt01to10/%s/AnaV_Sum_%d.root",dir.c_str(),i);
		else	name=Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/PFcandpt01to10/%s/AnaV_Prod_%d.root",dir.c_str(),i);
		remove(name.Data());
...
void dosecond(){
	...
	string dir=getenv("DIR");
        TString name;
        for(int i=start;i<end;i++){
                if(SumorProd=="Sum")       name=Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/PFcandpt01to10/%s/Anav_Prod_%d.root",dir.c_str(),i);
                else            name=Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/PFcandpt01to10/%s/Anav_Prod2_%d.root",dir.c_str(),i);
                remove(name.Data());
                LYZ *l = new LYZ(readline("filelistcrab.dat",i));
             	//LYZ *l = new LYZ(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
