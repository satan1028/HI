...
gr->GetYaxis()->SetRangeUser(5e-6,1e-1);
gr->GetXaxis()->SetRangeUser(0.05,0.3);
...
t->SetNDC();
if(xtheta==0)
t->DrawLatex(0.3,0.8,Form("%d < Ntrk <%d, #theta = %d, %.1f < p_{T} < %.1f", trkbin[xbin+1],trkbin[xbin],xtheta,pfptmin,pfptmax));
else
t->DrawLatex(0.3,0.8,Form("%d < Ntrk <%d, #theta = #frac{%d}{%d}#pi, %.1f < p_{T} < %.1f", trkbin[xbin+1],trkbin[xbin],xtheta,ntheta*nn,pfptmin,pfptmax));
