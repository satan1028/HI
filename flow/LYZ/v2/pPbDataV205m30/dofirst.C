...
void dofirst(){
	...
	for(int i=start;i<end;i++){
		if(isSum)	name=Form("/scratch/xuq7/flow/pbsjoboutput/pPbDataV205m30/AnaV_Sum_%d.root",i);
		else	        name=Form("/scratch/xuq7/flow/pbsjoboutput/pPbDataV205m30/AnaV_Prod_%d.root",i);
		remove(name.Data());
