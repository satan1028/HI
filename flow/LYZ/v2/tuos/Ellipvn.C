...
//void ellip()
{
  ...
  inR24.open("./v2.dat");  if(!inR24.good())   cout<<" input fail"<<endl;
  ...
  inR45.open("./v4.dat");  if(!inR45.good())   cout<<" input fail"<<endl;
  else cout<<" have put in"<<endl;
  for(i=0;i<Nbin;i++){
    inR45>>pt45[i];
    inR45>>v45[i];
  ...
  inR54.open("./v6.dat");  if(!inR54.good())   cout<<" input fail"<<endl;
  else cout<<" have put in"<<endl;
  for(i=0;i<Nbin;i++){
    inR54>>pt54[i];
    inR54>>v54[i];
  ...
  c1->SetTicks(-1);
   //c1->SetGrid();
  TGraphErrors *graph = new TGraphErrors(Nbin,pt24,v24,0,err24);
  graph->SetTitle("");
...
double ymax=0.95;
    ...
    leg0->SetTextSize(0.048);
    //leg0->SetLineStyle(0.06);
    leg0->AddEntry(graph," v_{2}","p");
    //leg0->AddEntry(g34," v_{3=13}","p");
    leg0->AddEntry(g45," v_{4}","p");
    leg0->AddEntry(g54," v_{6}","p");
    //leg0->AddEntry(g65," v_{6=23}","p");
    leg0->Draw();


   TLatex *tex = new TLatex(2.5.,0.2,"STEG,  10k");
   tex->SetTextColor(1);
   tex->SetTextFont(42);
