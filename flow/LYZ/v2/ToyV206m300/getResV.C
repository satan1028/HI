...
//------------------------Deal with the number and get the output stored in txt files and root files
	...
	TVectorD Gmod2[nbin][npt][ntheta];
	TVectorD sigma2[nbin][npt],deltaV[nbin][npt];
	TVectorD deltaVmean[nbin], Vmean[nbin];
	TVectorD r[nbin];
	TVectorD r0[nbin][npt], V[nbin][npt], chi[nbin][npt];
	TVectorD GRe[nbin][npt][ntheta]; TVectorD* GRe_t[nbin][npt][ntheta];
	TVectorD GIm[nbin][npt][ntheta]; TVectorD* GIm_t[nbin][npt][ntheta];
	TComplex G[nbin][npt][ntheta][nstepr];
	fstrV.open("V.txt");
	TFile *f[nFileAll];
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		avgmult[ibin].ResizeTo(npt);	deltaVmean[ibin].ResizeTo(npt);	Vmean[ibin].ResizeTo(npt);
		totpt[ibin].ResizeTo(npt);	totpt[ibin].Zero();
		totmult[ibin].ResizeTo(npt);	totmult[ibin].Zero();
		avgpt[ibin].ResizeTo(npt);
                Qx1[ibin].ResizeTo(npt);     Qx1[ibin].Zero();
                Qy1[ibin].ResizeTo(npt);     Qy1[ibin].Zero();
                Q2[ibin].ResizeTo(npt);     Q2[ibin].Zero();
	}

	for(int ifile=0; ifile<nFileAll; ifile++){
		f[ifile] = TFile::Open(Form("jobout/AnaV_Sum_%d.root",ifile));
	...
	TFile *outf = new TFile("mergedV_Sum.root","Recreate");
        Nevent.Write("Nevent");
