...
void getResv(){
	...
	ofstream fstrv;
	fstrv.open("v.txt");
	TVectorD totmult[nbin];	TVectorD Nevent, totmultall;
	...
	TVectorD avgmult[nbin][npt];	TVectorD avgmultall;
	TVectorD v[nbin][ntheta],vmean[nbin],deltav[nbin][ntheta],deltavmean[nbin];
	TFile *infile = TFile::Open("mergedV_Sum.root");
	...
	for(int ibin=0;ibin<nbin;ibin++){
        ...
        for(int ifile=0; ifile<nFileAll; ifile++){
	        f[ifile] = TFile::Open(Form("./jobout/Anav_Prod_%d.root",ifile));
	...
	for(int ibin=0;ibin<nbin;ibin++){
		...
		for(int iptbin=0;iptbin<npt; iptbin++){
			...
			for(int itheta=0;itheta<ntheta;itheta++){
				...
				deltav[ibin][itheta][iptbin]=TMath::Sqrt(TMath::Cos(mm*nn*theta[itheta])/totmultall[ibin]*(TMath::Exp(j01*j01/2./(*chi[ibin])[itheta]/(*chi[ibin])[itheta]*TMath::Cos(nn*theta[itheta]))*TMath::BesselJ0(2*j01*TMath::Sin(nn*theta[itheta]/2.))+TMath::Power(-1,mm)*TMath::Exp(-j01*j01/2./(*chi[ibin])[itheta]/(*chi[ibin])[itheta]*TMath::Cos(nn*theta[itheta]))*TMath::BesselJ0(2*j01*TMath::Cos(nn*theta[itheta]/2.))))/2./Besselj01(mm);
