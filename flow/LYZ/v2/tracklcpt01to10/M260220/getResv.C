...
void getResv(){
	...
	for(int ibin=0;ibin<nbin;ibin++){
        ...
        for(int ifile=0; ifile<nFileAll; ifile++){
	        if(SumorProd=="Sum") f[ifile] = TFile::Open(Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/tracklcpt01to10/%s/newptbin/Anav_Prod_%d.root",mdir.c_str(),ifile));
	        else f[ifile] = TFile::Open(Form("/lio/lfs/cms/store/user/qixu/flow/pbsjoboutput/tracklcpt01to10/%s/newptbin/Anav_Prod2_%d.root",mdir.c_str(),ifile));
