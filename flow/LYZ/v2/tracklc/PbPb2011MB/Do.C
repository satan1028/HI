...
void dofirst(){
	...
	for(int i=start;i<end;i++){
		if(SumorProd=="Sum")	name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/tracklc/AnaV_Sum_%d.root",i);
		else	name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/tracklc/AnaV_Prod_%d.root",i);
		remove(name.Data());
...
void dosecondsub(){
	...
	TFile *inf;
   	if(SumorProd=="Sum")       infname="mergedV_Sum_sub.root";
        ...
        for(int i=start;i<end;i++){
                if(SumorProd=="Sum")       name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/tracklc/Anav_Prod_sub_%d.root",i);
                else            name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/tracklc/Anav_Prod2_sub_%d.root",i);
                remove(name.Data());
                LYZ *l = new LYZ(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(1);
...
void dothird(){
        ...
        TString name;
        for(int i=start;i<end;i++){
                if(SumorProd=="Sum")       name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/finereta/tracklc/Anaveta_Prod_%d.root",i);
                else            name=Form("/scratch/xuq7/flow/pbsjoboutput/PbPb2011MB/finereta/tracklc/Anaveta_Prod2_%d.root",i);
                remove(name.Data());
                LYZ *l = new LYZ(readline("filelist.dat",i));
                cout<<"start "<<i<<" th job"<<endl;
                l->beginJob(0);
