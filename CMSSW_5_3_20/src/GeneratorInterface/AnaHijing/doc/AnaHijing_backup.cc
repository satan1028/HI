...
//if((*it)->status() != 1) cout<<(*it)->status()<<endl;
	 ...
	 int charge = static_cast<int>(part->charge());
        if(charge!=0&&pt>0.3&&fabs(eta)<5.0&&phi!=0){
...
*/
  ...
  hpt = f->make<TH1D>("pt","Tracks;pt",200,0,50);
  heta = f->make<TH1D>("eta","Tracks;eta",200,-4,4);
  hphi = f->make<TH1D>("phi","Tracks;phi",200,-4,4);
