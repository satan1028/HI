...
class UnmergedTrackAnalyzer : public edm::EDAnalyzer {
   public:
      explicit UnmergedTrackAnalyzer(const edm::ParameterSet&);
      ~UnmergedTrackAnalyzer();
   ...
   private:
      ...
      UnmergedTrackCorrector * corrector_;
...
UnmergedTrackAnalyzer::UnmergedTrackAnalyzer(const edm::ParameterSet& iConfig):
...
doPixel_(iConfig.getParameter<bool>("doPixel"))
{
  ...
  corrector_  = new UnmergedTrackCorrector();

  if( doPixel_ )
    corrector_->loadMap("MAP_validation3D_bass_hiConformalPixelTracks.par");
  else
    corrector_->loadMap();
...
UnmergedTrackAnalyzer::~UnmergedTrackAnalyzer()
{
}
...
void
UnmergedTrackAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   ...
   for ( ; tk != tracks->end(); ++tk )
   {
     if ( fabs( tk->eta() ) <= etaCut_ && tk->pt() >= ptMin_ )
     {
       ...
       double e = corrector_->effic( tk->pt(), tk->eta(), bin);
       double f = corrector_->fakeRate( tk->pt(), tk->eta(), bin);
...
void 
UnmergedTrackAnalyzer::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
UnmergedTrackAnalyzer::endJob() {
}

//define this as a plug-in
DEFINE_FWK_MODULE(UnmergedTrackAnalyzer);
