...
void SiStripDigitizerAlgorithm::run(edm::DetSet<SiStripDigi>& outdigi,
				    edm::DetSet<SiStripRawDigi>& outrawdigi,
                                    // --- BEGIN HACK --- //
				    edm::DetSet<SiStripRawDigi>& outrawsignaldigi,
				    edm::DetSet<SiStripRawDigi>& outrawnoisedigi,
                                    // --- END HACK --- //
  ...
  // --- BEGIN HACK --- //
  detNoiseAmpl.clear();
  detNoiseAmpl.insert(detNoiseAmpl.begin(),numStrips,0.);
  detSignalAmpl.clear();
  detSignalAmpl.insert(detSignalAmpl.begin(),numStrips,0.);
  // --- END HACK --- //
        ...
        for (size_t iChannel=localFirstChannel; iChannel<localLastChannel; iChannel++) {
          if(locAmpl[iChannel]>0.) {
			...
			detAmpl[iChannel]+=locAmpl[iChannel];
                        // --- BEGIN HACK --- //
                        detSignalAmpl[iChannel]+=locAmpl[iChannel];
                        // --- END HACK --- //
           }
        }
  ...
  // --- BEGIN HACK --- //
  //
  // For now, Signal collection will be zeroed by HIP
  for(int strip =0; strip < numStrips; ++strip) if(badChannels[strip]) detSignalAmpl[strip] =0;
  // --- END HACK --- //
  ...
  if(!zeroSuppression){
    ...
    // --- BEGIN HACK --- //
    for(int strip =0; strip < numStrips; ++strip) 
      detNoiseAmpl[strip] = detAmpl[strip] -  detSignalAmpl[strip];
    // --- END HACK --- //
    ...
    // --- BEGIN HACK --- //	
    rawsignaldigis.clear();
    rawsignaldigis = theSiDigitalConverter->convertRaw(detSignalAmpl, gainHandle, detID);
    outrawsignaldigi.data = rawsignaldigis;
    rawnoisedigis.clear();
    rawnoisedigis = theSiDigitalConverter->convertRaw(detNoiseAmpl, gainHandle, detID);
    outrawnoisedigi.data = rawnoisedigis;
    // --- END HACK --- //
