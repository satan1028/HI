...
//
//  This class has been adapted from the SimpleTrackListMerger.cc
...
namespace cms
{
  ...
  HiTrackListMerger::HiTrackListMerger(edm::ParameterSet const& conf) : 
    conf_(conf)
  {
  ...
  HiTrackListMerger::~HiTrackListMerger() { }  

  // Functions that gets called by framework every event
  void HiTrackListMerger::produce(edm::Event& e, const edm::EventSetup& es)
  {
  ...
  // 
   ...
   if ( 0<tC1.size() ){
      ...
      for (reco::TrackCollection::const_iterator track=tC1.begin(); track!=tC1.end(); track++){
        ...
        if ( track->pt() < tC1MinPt || track->pt() > tC1MaxPt ){
          selected1[i]=0;
        }
   ...
   if ( 0<tC2.size() ){
      ...
      for (reco::TrackCollection::const_iterator track=tC2.begin(); track!=tC2.end(); track++){
        ...
        if ( track->pt() < tC2MinPt || track->pt() > tC2MaxPt ){
          selected2[i]=0;
        }
      }//end loop over tracks
   }//end more than 0 track
