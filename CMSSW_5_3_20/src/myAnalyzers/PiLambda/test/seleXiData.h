...
public :
   ...
   void DoNumFit(std::vector<TH1F*>, TH1F*, std::string, bool=false);
...
#ifdef seleXiData_cxx
seleXiData::seleXiData(TTree *tree){
...
seleXiData::~seleXiData(){
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t seleXiData::GetEntry(Long64_t entry){
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}

Long64_t seleXiData::LoadTree(Long64_t entry){
...
void seleXiData::Init(TTree *tree){
...
void seleXiData::Show(Long64_t entry){
...
void seleXiData::SetNEvents(int theNumberOfEvents){
  nEvents = theNumberOfEvents;
}

void seleXiData::WriteHistos(){
  theOutRootFile->Write();
  theOutRootFile->Close();
}

void seleXiData::SetCutValues(float xi3dCut, float batpi3dCut, float v0pi3dCut, float v0proton3dCut, float sepCut, float vxtclCut){
...
Int_t seleXiData::Cut(Long64_t entry){
