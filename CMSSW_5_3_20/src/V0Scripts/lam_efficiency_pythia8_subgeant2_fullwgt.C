...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  if ( weight_choice >= 100 ) {
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	wgt = wgt*y_wgt*pt_wgt;
	if (processType != 103 && processType != 104 && (*lamgenMass)[i] < 1.115682 ) {
	  lam_dNdy_gen->Fill(y,wgt);
	  lam_dNdpT_gen->Fill(pt,wgt);
	  lam_dNdlife_gen->Fill(life,wgt);
	}
	...
	if( y > 2.3 ) continue;
	if ( (*lamgenMass)[i] > 1.115682 ) wgt = wgt*2.0;
	life = (*lamrecoCtau)[i];
	lam_dNdy_rec->Fill(y,wgt);
	lam_dNdpT_rec->Fill(pt,wgt);
	lam_dNdlife_rec->Fill(life,wgt);
