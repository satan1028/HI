#define lam_efficiency_subgeant2_cxx
#include "lam_efficiency_subgeant2.h"
#include <TH2.h>
#include <TH1F.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void lam_efficiency_subgeant2::Loop(int weight_choice,
			  TH1F* veematch,
			  TH1F* lam_dNdy_gen,
			  TH1F* lam_dNdy_rec,
			  TH1F* lam_dNdpT_gen,
...
//      Root > .L lam_efficiency_subgeant2.C
//      Root > lam_efficiency_subgeant2 t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  if ( weight_choice >= 100 ) {
   ...
   double p, pt, y, life, mass, p2, pz, energy, wgt, y_wgt, pt_wgt;
   int ncand, wgt_scale;
   bool lambda;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if (processType != 92 && processType != 93 && processType != 103 && 
	    processType != 104 && (*lamgenMass)[i] < 1.115682 ) {
	  lam_dNdy_gen->Fill(y,wgt*y_wgt);
	  lam_dNdpT_gen->Fill(pt,wgt*pt_wgt);
	  lam_dNdlife_gen->Fill(life,wgt*y_wgt);
	  if ( lambda ) {
	    lamReg_dNdy_gen->Fill(y,wgt*y_wgt);
	    lamReg_dNdpT_gen->Fill(pt,wgt*pt_wgt);
	    lamReg_dNdlife_gen->Fill(life,wgt*y_wgt);
	  } else {
	    lamBar_dNdy_gen->Fill(y,wgt*y_wgt);
	    lamBar_dNdpT_gen->Fill(pt,wgt*pt_wgt);
	    lamBar_dNdlife_gen->Fill(life,wgt*y_wgt);
	  }
	}
	...
	if( y > 2.3 ) continue;
	life = (*lamrecoCtau)[i];
	wgt_scale = ((*lamgenMass)[i] > 1.115682) ? 2 : 1;
	lam_dNdy_rec->Fill(y,wgt_scale*wgt*y_wgt);
	lam_dNdpT_rec->Fill(pt,wgt_scale*wgt*pt_wgt);
	lam_dNdlife_rec->Fill(life,wgt_scale*wgt*y_wgt);
	if ( lambda ) {
	  lamReg_dNdy_rec->Fill(y,wgt_scale*wgt*y_wgt);
	  lamReg_dNdpT_rec->Fill(pt,wgt_scale*wgt*pt_wgt);
	  lamReg_dNdlife_rec->Fill(life,wgt_scale*wgt*y_wgt);
	} else {
	  lamBar_dNdy_rec->Fill(y,wgt_scale*wgt*y_wgt);
	  lamBar_dNdpT_rec->Fill(pt,wgt_scale*wgt*pt_wgt);
	  lamBar_dNdlife_rec->Fill(life,wgt_scale*wgt*y_wgt);
	}
      }
   }
}
