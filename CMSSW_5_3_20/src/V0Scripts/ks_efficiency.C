...
void ks_efficiency::Loop(int weight_choice,
			 ...
			 TH1F* ks_dNdlife_gen,
			 TH1F* ks_dNdlife_rec,
			 TH1F* ks_dNdy_neg_gen,
			 TH1F* ks_dNdy_neg_rec,
			 TH1F* ks_dNdy_pos_gen,
			 TH1F* ks_dNdy_pos_rec)
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  if ( weight_choice >= 100 ) {
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if ( y > 2.3 ) continue;
	life = (*ksgenCtau)[i];
	y_wgt = kspt_wgt( pt, kin_weight_choice );
	pt_wgt = ksy_wgt( y, kin_weight_choice );
	if (processType != 92 && processType != 93 && processType != 103 && 
	    processType != 104 ) {
	  ks_dNdy_gen->Fill(y,wgt*y_wgt);
	  if ( (*ksgenEta)[i] < 0.0 ) {
	    ks_dNdy_neg_gen->Fill(y,wgt*y_wgt);
	  } else {
	    ks_dNdy_pos_gen->Fill(y,wgt*y_wgt);
	  }
	...
	if ( pz < 0.0 ) {
	  ks_dNdy_neg_rec->Fill(y,wgt*y_wgt);
	} else {
	  ks_dNdy_pos_rec->Fill(y,wgt*y_wgt);
	}
