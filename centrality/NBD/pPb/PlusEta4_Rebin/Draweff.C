...
void Draweff(){
	...
	TFile *ftreff = TFile::Open("/afs/cern.ch/work/q/qixu/private/RpA/GlobalEvent/CentrDep/pPbHist_Hijing_HLT_CutGplus_pPA_pprim.root");
	TH1D* hbef = (TH1D*)ftreff->Get("hHFEnergyPlus4");
	TH1D* rehbef = (TH1D*)hbef->Rebin(nhfbin,"rehHFEnergyPlus4",hfbin);
	TH1D* haft = (TH1D*)ftreff->Get("hHFEnergyPlus4_tr");
	TH1D* rehaft = (TH1D*)haft->Rebin(nhfbin,"rehHFEnergyPlus4_tr",hfbin);
	TGraphAsymmErrors *gtreff = new TGraphAsymmErrors();
	gtreff->BayesDivide(rehaft,rehbef);

	TGraphAsymmErrors *geff = (TGraphAsymmErrors*)fDSeff->Get("regEffHFp4");
