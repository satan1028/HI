...
void Draw3panelfit(){
	...
	histo_obs_norm->SetMarkerColor(1);
        ...
        leg->AddEntry(histo_exp_norm,Form("NBD Fit","l"));
	if(sth==2) leg->Draw("same");
        ...
        TLatex *tex3= new TLatex(0.60,0.83,Form("#mu=%.3f, k=%.3f",(*mubest)[0],(*kbest)[0]));
	...
	c1->SaveAs(Form("%s_3panelfit.png",cenvarlist[cenvar].Data()));	
	c1->SaveAs(Form("%s_3panelfit.pdf",cenvarlist[cenvar].Data()));	
}
