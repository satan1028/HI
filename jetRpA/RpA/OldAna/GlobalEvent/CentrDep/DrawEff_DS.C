...
h_or[k][j]=(TH1F*)f[k]->Get(histoname[j]);
h_tr[k][j]=(TH1F*)f[k]->Get(Form("%s_tr",histoname[j].Data()));
h_ds[k][j]=(TH1F*)f[k]->Get(Form("%s_DS",histoname[j].Data()));
...
gEff[k][j]=new TGraphAsymmErrors();	gEff[k][j]->SetName(Form("gEff_%s%s",Gen[k].Data(),histoname[j].Data()));
...
gEff[k][j]->GetXaxis()->SetLimits(0.1,99.9);
gEff[k][j]->SetTitle("");
}
}
...
leg->AddEntry(gEff[k][j],etarange[j],"p");
}
