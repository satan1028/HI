...
const int Nbin_var=100;
double binbound_var[101];
...
TString filename2=Form("NoGplusCombinedJetTrigak3PFJetAllHist.root");
...
TString histoname="HFp4";
TString histo;
if(histoname=="CentWeighted")
histo="hiBin";
else if(histoname=="HFp4")
histo="HF E_{T} #eta>4 (GeV/c)";
else if(histoname=="HF4")
histo="HF E_{T} |#eta|>4 (GeV/c)";

TFile *file1=TFile::Open(Form("%s/%s",dir1.Data(),filename1.Data()));
TFile *file2=TFile::Open(Form("%s/%s",dir2.Data(),filename2.Data()));

TH1D* histo1=(TH1D*)file1->Get((TString)("pfjet_"+histoname));
TH1D* histo2=(TH1D*)file2->Get((TString)("pfjet_"+histoname));

rehisto1=(TH1D*)histo1->Clone("rehisto1");
rehisto2=(TH1D*)histo2->Clone("rehisto2");
rehisto1=(TH1D*)rehisto1->Rebin(Nbin_var,"rehisto1",binbound_var);
rehisto2=(TH1D*)rehisto2->Rebin(Nbin_var,"rehisto2",binbound_var);
...
makeMultiPanelCanvas(c1,1,2,-0.12,0,0.16,0.14,0.03);
if(gen=="PP")
...
//T1->Draw("same");
if(gen=="PP"){
c1->Print(Form("CompDataMC%sPP.png",histoname.Data()));
c1->Print(Form("CompDataMC%sPP.pdf",histoname.Data()));
}
else if(gen=="PPb"){
c1->Print(Form("CompDataMC%sPPb.png",histoname.Data()));
c1->Print(Form("CompDataMC%sPPb.pdf",histoname.Data()));
}
