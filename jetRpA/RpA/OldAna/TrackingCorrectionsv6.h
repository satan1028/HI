...
void TrackingCorrections::Init()
{
   ...
   cout << " Setup - smoothLevel: " << smoothLevel_ << " collMode: " << coll_ << " trkPhiMode: " << trkPhiMode_;
...
//             hnameNoE = Form("hPtHatBeforeSel");
   ...
   for (UInt_t s=0; s<ptHatMin_.size(); ++s) { // merge pt hat samples with weight
     ...
     */ // for pp@5TeV cross section weight
       if ( ptHatMin_[s]==50 ) sampleCroSec_[s] = 3.778E-03 - 4.412E-04;
       else if ( ptHatMin_[s]==80 ) sampleCroSec_[s] = 4.412E-04 - 6.147E-05;
         ...
         if (coll_=="PPb") {
      // for ppb@5TeV cross section weight
       if ( ptHatMin_[s]==50 ) sampleCroSec_[s] = 3.778E-03 - 4.412E-04;
       else if ( ptHatMin_[s]==80 ) sampleCroSec_[s] = 4.412E-04 - 6.147E-05;
        else if ( ptHatMin_[s]==120) sampleCroSec_[s] = 6.147E-05 - 1.018E-05;
        else if ( ptHatMin_[s]==170) sampleCroSec_[s] = 1.018E-05 - 2.477E-06; //upper limit is 220
        else if ( ptHatMin_[s]==220) sampleCroSec_[s] = 2.477E-06 - 6.160E-07;
        else if ( ptHatMin_[s]==280) sampleCroSec_[s] = 6.160E-07 - 1.088E-07;
        else if ( ptHatMin_[s]==370) sampleCroSec_[s] = 1.088E-07;
      else cout << endl << endl << " Error no such pt hat!!!!!" << endl << endl << endl;
      }
     float basePtHat;
...
Float_t TrackingCorrections::GetCorr(Float_t pt, Float_t eta, Float_t jet, Float_t cent, Double_t * outCorr)
{
   ...
   // if eff==0, no correction, b/c too few statistics   
   if(corr[0]<0.001) {
       if(pt > 20. ) corr[0] = correctionHists_[0][bin]->GetBinContent(etaBin,ptBin-1,jetBin);
       else if(jet >40. ) corr[0] = correctionHists_[0][bin]->GetBinContent(etaBin,ptBin,jetBin-1);
       else corr[0] = correctionHists_[0][bin]->GetBinContent(etaBin,ptBin, 0);
       cout <<"taking the neighbour bin efficiency since original eff too small, neighbour ="<< corr[0] <<endl ;
    }
   if (corr[0]<0.001) {
...
TH1 * TrackingCorrections::InspectCorr(Int_t lv, Int_t centBeg, Int_t centEnd, Int_t jetBegBin, Int_t jetEndBin, Int_t mode, Int_t begbin, Int_t endbin)
{
   ...
   for (UInt_t s=0; s<ptHatMin_.size(); ++s) { // merge pt hat samples with weight                                                             
      for (Int_t c=0; c<numCentBins_; ++c) {
         ...
         if (coll_=="PPb" || coll_=="PP") hname = "hPtHat";
