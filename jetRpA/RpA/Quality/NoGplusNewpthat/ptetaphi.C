...
bool isMC=kFALSE;
bool Save=kFALSE;
TString histoname[2]={"jetptEta_0-100%","jetptphi_0-100%"};
void ptetaphi(){
...
TH2F* hdata=(TH2F*)fdata->Get(histoname[ivar]);
...
if(xrange_pt[0]<110)
hFrame1->GetYaxis()->SetRangeUser(1.01e-4,2e-3);
else{
if(varname!="jet #eta_{lab}")
hFrame1->GetYaxis()->SetRangeUser(1.01e-5,4e-5);
}
...
if(Save){
if(isMC){
c1->Print(Form("pic/DataMC/PPbCompPP_%.fto%.f.gif",xrange_pt[0],xrange_pt[1]));
c1->Print(Form("pic/DataMC/PPbCompPP_%.fto%.f.pdf",xrange_pt[0],xrange_pt[1]));
}
else{
c1->Print(Form("pic/DataMC/DataPPbCompPP_%.fto%.f.gif",xrange_pt[0],xrange_pt[1]));
c1->Print(Form("pic/DataMC/DataPPbCompPP_%.fto%.f.pdf",xrange_pt[0],xrange_pt[1]));
}
}
