...
TString filename=Form("MC%sNoGplus_akPu3PFDiJetMassJetNoResidualPt30GenChargeTrkEffHIN12017v5TrkCorr2DCut0EtaBin7_HFsumEta4Bin1_merged.root",gen.Data());
TString dir="~/CMSSW_6_2_3_patch1/src/jetRpA/RpA/output/JetTrig/JetID/akPu3PF/NoResidual/NoGplusNewpthat/newSample";
TGraphAsymmErrors *g;
...
TFile *file=TFile::Open(Form("%s/%s",dir.Data(),filename.Data()));
double jetptcut=29;
TH1F* ratio;
...
hFrame->GetYaxis()->SetTitle(Form("Ratio: jetpt>=%.f/nocut",jetptcut));
...
TH2F* histo2F=(TH2F*)file->Get(Form("refptpt_0-100%%"));
...
TH1F* histo4=(TH1F*)histo2F->ProjectionY("histo4",histo2F->GetXaxis()->FindBin(47),histo2F->GetXaxis()->FindBin(55));
/*
...
T1->SetTextFont(42);
if(gen=="PP")
T1->DrawLatex(0.5,0.92,"PYTHIA");
else if(gen=="PPb")
T1->DrawLatex(0.5,0.92,"PYTHIA+HIJING");
//T1->DrawLatex(0.25,0.8,"47<=p_{T}^{ref}<55 (GeV/c)");
...
handlehisto(histo3);
handlehisto(histo4);
histo4->GetXaxis()->SetRangeUser(0,100);
histo4->GetYaxis()->SetRangeUser(0,0.1);
...
l0->Draw("same");
T1->Draw("same");
TString outname="refptpt";
if(gen=="PP"){
c1->Print(Form("%sPP.png",outname.Data()));
c1->Print(Form("%sPP.pdf",outname.Data()));
}
else if(gen=="PPb"){
