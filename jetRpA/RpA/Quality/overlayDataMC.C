...
TH1D* hdata=(TH1D*)hdata2F->ProjectionY("hdata",hdata2F->GetXaxis()->FindBin(xrange_pt[0]),hdata2F->GetXaxis()->FindBin(xrange_pt[1]),"e");
TH1D* hPPb=(TH1D*)hPPb2F->ProjectionY("hPPb",hPPb2F->GetXaxis()->FindBin(xrange_pt[0]),hPPb2F->GetXaxis()->FindBin(xrange_pt[1]),"e");
...
rehdata->Scale(1./rehdata->Integral());
rehPPb->Scale(1./rehPPb->Integral());
...
fixedFontHist(rehdata,1.6,2.2);
fixedFontHist(rehPPb,1.6,2.2);

c1 = new TCanvas("c1","",400,700);
makeMultiPanelCanvas(c1,1,2,-0.05,0,0.14,0.12,0.03);
