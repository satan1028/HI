...
hFrame->GetXaxis()->SetLimits(47.5,150);
hFrame->GetYaxis()->SetRangeUser(0.99,1.005);
hFrame->DrawCopy();
...
TH1F* histo1=(TH1F*)fMCPPb->Get(Form("refjetptEtaBin%s",etabinname[i].Data()));
TH1F* histo2=(TH1F*)fMCPPb->Get(Form("genjetptEtaBin%s",etabinname[i].Data()));
}
...
TLine *l =new TLine(47.5,1,150,1);
l->SetLineStyle(2);
l->SetLineColor(1);
}
for(int i=0;i<Neta-1;i++)	if(i!=3) g0[i]->Draw("Psame");
//for(int i=2;i<3;i++)	ratio[i]->Draw("Psame");
leg1->Draw("same");
l->Draw("same");
T1->DrawLatex(0.2,0.8,"PYTHIA+HIJING");
T1->DrawLatex(0.2,0.85,"CMS simulation");
