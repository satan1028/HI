...
c1->cd();
TH1F* ratio[Neta];
TGraphAsymmErrors *g[Neta];
TGraphAsymmErrors *g0[Neta];
...
hFrame->GetXaxis()->SetLimits(30,200);
hFrame->GetYaxis()->SetRangeUser(0.6,1.06);
hFrame->DrawCopy();
...
if(i==Neta-1){
TH1F* histo1=(TH1F*)fMCPPb->Get(Form("refjetpt"));
TH1F* histo2=(TH1F*)fMCPPb->Get(Form("genjetpt"));
}
else{
TH1F* histo1=(TH1F*)fMCPPb->Get(Form("refjetptEtaBin%s",etabinnameswap[i].Data()));
TH1F* histo2=(TH1F*)fMCPPb->Get(Form("genjetptEtaBin%s",etabinnameswap[i].Data()));
}
...
TLine *l =new TLine(30,1,200,1);
l->SetLineStyle(2);
l->SetLineColor(1);
}
for(int i=0;i<Neta-1;i++)	if(i!=3&&canvas[i]!=4) g0[i]->Draw("Psame");
//for(int i=2;i<3;i++)	ratio[i]->Draw("Psame");
leg1->Draw("same");
l->Draw("same");
T1->DrawLatex(0.25,0.7,"PYTHIA+HIJING");
T1->DrawLatex(0.25,0.75,"CMS simulation");
T1->DrawLatex(0.4,0.2,"Anti-k_{T} Particle Flow PU Jets: R=0.3");

//c1->Print("pic/centWeight/jeteffPPb.png");
c1->Print("jeteffPPb.pdf");
