...
nt->GetEntry(i);
	...
	if(trawpt[j4i]<22.||TMath::Abs(tjeteta[ij])>0.3) continue ;
...
ntold->GetEntry(iold);
	if(trawptold<22.||TMath::Abs(tjetetaold)>0.3) continue ;
...
*/
nt->Draw("jtpt>>jetpt","rawpt>22 && TMath::Abs(jteta)<=3 && TMath::Abs(vz)<=15 && pHBHENoiseFilter && pprimaryvertexFilter && pPAcollisionEventSelectionPA && (HLT_PAJet20_noJetID_v1 || HLT_PAJet40_noJetID_v1 || HLT_PAJet60_noJetID_v1 || HLT_PAJet80_noJetID_v1 || HLT_PAJet100_noJetID_v1)");
ntold->Draw("jtpt>>jetptold","rawpt>22 && TMath::Abs(jteta)<=3");
nt->Draw("jteta>>jeteta","rawpt>22 && TMath::Abs(jteta)<=3 && TMath::Abs(vz)<=15 && pHBHENoiseFilter && pprimaryvertexFilter && pPAcollisionEventSelectionPA && (HLT_PAJet20_noJetID_v1 || HLT_PAJet40_noJetID_v1 || HLT_PAJet60_noJetID_v1 || HLT_PAJet80_noJetID_v1 || HLT_PAJet100_noJetID_v1)");
ntold->Draw("jteta>>jetetaold","rawpt>22 && TMath::Abs(jteta)<=3");
nt->Draw("jtphi>>jetphi","rawpt>22 && TMath::Abs(jteta)<=3 && TMath::Abs(vz)<=15 && pHBHENoiseFilter && pprimaryvertexFilter && pPAcollisionEventSelectionPA && (HLT_PAJet20_noJetID_v1 || HLT_PAJet40_noJetID_v1 || HLT_PAJet60_noJetID_v1 || HLT_PAJet80_noJetID_v1 || HLT_PAJet100_noJetID_v1)");
ntold->Draw("jtphi>>jetphiold","rawpt>22 && TMath::Abs(jteta)<=3");
fout->cd();
jetpt->Write();
jeteta->Write();
jetphi->Write();
