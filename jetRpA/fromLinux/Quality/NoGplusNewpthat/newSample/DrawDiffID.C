...
rehisto->GetXaxis()->SetRangeUser(20,600);
...
makeMultiPanelCanvas(c6,2,1,0.03,0.02,0.08,0.10,0.03);
...
c6->cd(1);
hFrame->DrawCopy();
TLegend *leg1=new TLegend(0.76,0.13,0.91,0.43);
TLegend *leg2=new TLegend(0.73,0.13,0.88,0.43);

for(int j=0;j<N;j++){
if(j==0 || j==2 || j==5){
hPP2D[j]= (TH2F*)fPP->Get(Form("jetpt%s_0-100%%",JetIDNameList_Simple[j].Data()));
hPPb2D[j]= (TH2F*)fPPb->Get(Form("jetpt%s_0-100%%",JetIDNameList_Simple[j].Data()));
hPP2D_fake[j]= (TH2F*)fPP->Get(Form("jetpt%s_fake_0-100%%",JetIDNameList_Simple[j].Data()));
hPPb2D_fake[j]= (TH2F*)fPPb->Get(Form("jetpt%s_fake_0-100%%",JetIDNameList_Simple[j].Data()));
...
hPP_pt[j]=hPP2D[j]->ProjectionX(Form("hPP_pt_%d",j));
hPP_pt_fake[j]=hPP2D_fake[j]->ProjectionX(Form("hPP_pt_fake_%d",j));
hPP_JetIDcut_pt[j]=hPP2D[j]->ProjectionX(Form("hPP_JetIDcut_pt_%d",j),hPP2D[j]->GetYaxis()->FindBin(xrange_JetIDcut[j][0]),hPP2D[j]->GetYaxis()->FindBin(xrange_JetIDcut[j][1]));
hPP_JetIDcut_pt_fake[j]=hPP2D_fake[j]->ProjectionX(Form("hPP_JetIDcut_pt_fake_%d",j),hPP2D_fake[j]->GetYaxis()->FindBin(xrange_JetIDcut[j][0]),hPPb2D_fake[j]->GetYaxis()->FindBin(xrange_JetIDcut[j][1]));

//---------------------------------------------------Rebin and Format histograms-----------------------------------------

rehisto_hPP_pt[j]=myRebin(hPP_pt[j],Nbin_pt,binbound_pt);
rehisto_hPP_pt_fake[j]=myRebin(hPP_pt_fake[j],Nbin_pt,binbound_pt);
rehisto_hPP_JetIDcut_pt[j]=myRebin(hPP_JetIDcut_pt[j],Nbin_pt,binbound_pt);
rehisto_hPP_JetIDcut_pt_fake[j]=myRebin(hPP_JetIDcut_pt_fake[j],Nbin_pt,binbound_pt);
...
ratio_hPPb_JetIDcutvsnocut_pt[j]->Draw("same");
...
ratio_hPPb_JetIDcutvsnocut_pt_fake[j]->Draw("same");
}
...
c6->Print(Form("pic/JetIDcut/%s/ratio_jetpt_cutvsnocut.png",algo.Data()));
