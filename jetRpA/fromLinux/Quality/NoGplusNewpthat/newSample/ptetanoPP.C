...
bool isMC=kFALSE;
bool Save=kTRUE;
TString histoname[2]={"jetptsubidEta_0-100%","jetptphi_0-100%"};
TString histonamedata[2]={"jetptEta","jetptphi"};
void ptetanoPP(){
...
re_hPPb_pt->SetMarkerStyle(0);

re_hPPb_var->SetLineColor(2);
re_hPPb_var->SetFillStyle(3004);
re_hPPb_var->SetMarkerStyle(0);
...
T.SetTextColor(1);
T.SetTextFont(42);
TH1F* hFrame=new TH1F("","",10000,0,10000);
...
if(Save){
	...
	hFrame->DrawCopy();
	//ratio_DatavsPPb_pt->SetMarkerStyle(2);
...
re_hPPb_var->DrawCopy("E1HIST same");
if(varname!="jet #eta_{lab}")
leg1->Draw("same");
...
if(Save){
if(isMC){
c1->Print(Form("PPbComp_%.fto%.f.gif",xrange_pt[0],xrange_pt[1]));
c1->Print(Form("PPbComp_%.fto%.f.pdf",xrange_pt[0],xrange_pt[1]));
}
else{
c1->Print(Form("DataPPbComp_%.fto%.f.gif",xrange_pt[0],xrange_pt[1]));
c1->Print(Form("DataPPbComp_%.fto%.f.pdf",xrange_pt[0],xrange_pt[1]));
}
}
