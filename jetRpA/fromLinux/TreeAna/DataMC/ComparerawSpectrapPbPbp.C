...
//  int ieta=6; //0 to 7
 ...
 // TFile *f1 = new TFile(Form("/home/xuq/Documents/HI/RpA/output/UnfoldedOutputs/PPb_UnfoPriorGen_akPu3PFKurtForestEtaWeightNoIDCut_jtpt20_EtaBin%.f_%.f_Inc_v13.root",RevEtaMin[ieta]*10,RevEtaMax[ieta]*10));//PPb no residual unfolding
  TFile *f1 = new TFile(Form("/home/xuq/Documents/HI/RpA/output/UnfoldedOutputs/MCPPbakPu3PFJetSpectraCombineTriggerNoWeightNoJetIDCutRawPt.root"));//PPb no residual unfolding
  TFile *f2 = new TFile(Form("/home/xuq/Documents/HI/RpA/output/UnfoldedOutputs/MCPbPakPu3PFJetSpectraCombineTriggerNoWeightNoJetIDCutRawPt.root"));//PbP no residual unfolding

  
 TH1F *h1eta = (TH1F*)f1->Get(Form("jetptEtaBin%.f_%.f",RevEtaMin[ieta]*10,RevEtaMax[ieta]*10));
 TH1F *h2eta = (TH1F*)f2->Get(Form("jetptEtaBin%.f_%.f",EtaMin[ieta]*10,EtaMax[ieta]*10));
...
//-----------------------------------ratio-----------------------------------
  ...
  if(SavePlots) {
    c1->SaveAs(Form("CompareRawSpectrapPb_PbpEta%.f_%.f.pdf",EtaMin[ieta]*10,EtaMax[ieta]*10));
   }
