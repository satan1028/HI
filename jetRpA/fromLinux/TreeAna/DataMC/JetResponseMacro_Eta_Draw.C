...
//  gROOT->ProcessLine(".x rootlogon.C");
  ...
  TString inname = "JetResponse_Plots.root"; 
  TFile* FileA = new TFile(inname);
    TString plotsdir = "./";
  ...
  //  makeMultiPanelCanvas(c1,nBin/4,2,0.0,0.0,0.1,0.2,0.05);
    makeMultiPanelCanvas(c1,npt,1,0.0,0.0,0.1,0.2,0.05);
...
//  makeMultiPanelCanvas(c1,cCanDiv,2,0.0,0.0,0.2,0.15,0.07);
  ...
  TLegend*  leg[npt];
    ...
    makeMultiPanelCanvas(c2,4,1,0.0,0.0,0.1,0.2,0.05);
 //   Int_t i = 2 ;
  ...
  //  for(int i =1; i<nCanvDiv-1; i++)
    {
      ...
      leg[i]->SetBorderSize(0);
        ...
        iHistoCorrEta[i] = (TH2D*)FileA->Get(Form("%s_CorrEtaInpt%d",AlgoNames[i],iptbin));
        iHistoCorrEta[i+6] = (TH2D*)FileA->Get(Form("%s_CorrEtaInpt%d",AlgoNames[i+6],iptbin));

    //c1->cd(iptbin+1);
     //   iHistoCorrEta[i]->Draw("colz");
     //   iHistoCorrEta[i]->Draw("same colz");
        leg[i]->AddEntry(iHistoCorrEta[i],Form("%s",AlgoNames[i]),"");
        leg[i]->AddEntry(iHistoCorrEta[i+6],Form("%s",AlgoNames[i+6]),"");
        ...
        iHistoCorrEta[i+6]->SetTitle("");
        iHistoCorrEta[i]->SetTitle("");
       // leg[i]->Draw();
        for(int iX=iHistoCorrEta[i]->GetXaxis()->GetFirst(); iX<iHistoCorrEta[i]->GetXaxis()->GetLast(); iX++ )
        {
            ...
            h1[i][iX] = new TH1D(Form("h1_%d_%d_%dpy",iptbin,i,iX),Form("h1_%d_%d_%d_py",iptbin,i,iX),500,0,5);
            h1[i][iX] =(TH1D*)iHistoCorrEta[i]->ProjectionY(Form("h1_%d_%d_%dpy",iptbin,i,iX),iX,iX,"e");
  ...
  //          Gauss6[i][iX]->SetParLimits(1,0.7,1.5);
   //         h1[i][iX]->Fit(Form("F6_c%d_d%d",i,iX),"RQ0WL", "", 0.7, 1.5);
            ...
            h1[i+6][iX] = new TH1D(Form("h1_%d_%d_%dpy",iptbin,i+6,iX),Form("h1_%d_%d_%d_py",iptbin,i+6,iX),500,0,5);
            h1[i+6][iX] =(TH1D*)iHistoCorrEta[i+6]->ProjectionY(Form("h1_%d_%d_%dpy",iptbin,i+6,iX),iX,iX,"e");
...
*/    
 ...
 //   makeMultiPanelCanvas(c2,nBin/4,2,0.0,0.0,0.1,0.2,0.05);
  ...
  //  makeMultiPanelCanvas(c3,4,1,0.0,0.0,0.1,0.2,0.05);
  ...
  for(int i2 =0; i2<1; i2++)
    {
      leg2A[i2]= new TLegend(0.12,0.75,0.35,0.92);//top right
...
CorrPt_Mean[i2]->SetTitle();
CorrPt_Mean[i2]->SetTitle();
      CorrPt_Mean[i2]->SetName(Form("CorrPt_Mean%d",iptbin));
      CorrPt_Mean[i2+6]->SetName(Form("CorrPt_Mean1%d",iptbin));
      leg2A[i2]->AddEntry(CorrPt_Mean[i2],Form("%s",AlgoNames[i2]),"PL");
        leg2A[i2]->AddEntry(CorrPt_Mean[i2+6],Form("%s",AlgoNames[i2+6]),"PL");
  //  leg2A[i2]->AddEntry(CorrPt_Mean[i2],Form(" %d <p_{T}^{ref} < %d GeV/c",ptbinlow[iptbin],ptbinhigh[iptbin]),"");
      leg2A[i2]->AddEntry(CorrPt_Mean[i2],Form("p_{T}^{corrected}/p_{T}^{ref}"),"");
     CorrPt_Mean[i2]->GetXaxis()->SetRangeUser(-3., 3.);
      CorrPt_Mean[i2]->SetMaximum(1.12);
      CorrPt_Mean[i2]->SetMinimum(0.85);
...
if(iptbin==0) {     leg2A[i2]->Draw();
	drawText(Form("%d < p_{T}^{ref} < %d",ptbinlow[iptbin],ptbinhigh[iptbin]),0.65,0.80);
}
else
	drawText(Form("%d < p_{T}^{ref} < %d",ptbinlow[iptbin],ptbinhigh[iptbin]),0.55,0.80);
        regSun(-3.,1.,3.,1.,1, 1);
    c2->Update();
}
}
...
/*
    ...
    c3->Update();
*/
        c2->Print(Form("Pbp502CorrectedEtaOverRefJetMean.pdf", plotsdir.Data()));
