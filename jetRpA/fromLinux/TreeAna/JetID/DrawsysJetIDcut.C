...
TH1D* histo=(TH1D*)hdata2F->ProjectionX("histo",hdata2F->GetYaxis()->FindBin(JetIDcut[0]),hdata2F->GetXaxis()->FindBin(JetIDcut[1]));
...
if(histo0->GetBinCenter(ibin)>25 && histo0->GetBinCenter(ibin)<=600){
fstr[i]<<histo0->GetBinCenter(ibin)<<'\t';
if(histo0->GetBinContent(ibin)!=0){
...
//c1->Print(Form("/afs/cern.ch/user/q/qixu/CMSSW_6_2_5/src/RpA/skimTree/AccreTreeAna/JetID/pic/JetIDcutsys%s.png",etabinname[i].Data()));
c1->Print(Form("/afs/cern.ch/user/q/qixu/CMSSW_6_2_5/src/RpA/skimTree/AccreTreeAna/JetID/pic/JetIDcutsys%s.pdf",etabinname[i].Data()));
}
}
