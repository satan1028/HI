...
else if(ilist== 12) {    JetID = "(#Sigma h^{#pm}+#Sigma #gamma +#Sigma h^{0}+#Sigma #mu+#Sigma e)/p_{T}^{jet}";JetIDcut[0]=0; JetIDcut[1]=2;}
...
cout<<"ptlowbin to pthighbin: "<<"\t\t"<<"PP fake Entries"<<"\t\t"<<"PPb fake Entries"<<"\t\t"<<"Total jets"<<endl;
for(int i=1;i<rehisto_hProfPP_fake->GetXaxis()->GetNbins();i++){
cout<<rehisto_hProfPP_fake->GetXaxis()->GetBinLowEdge(i)<<" to "<<rehisto_hProfPP_fake->GetXaxis()->GetBinUpEdge(i)<<" : "<<rehisto_hProfPP_fake->GetBinEffectiveEntries(i)<<"\t\t"<<rehisto_hProfPPb_fake->GetBinEffectiveEntries(i)<<"\t\t"<<rehisto_hProfPPb->GetBinEffectiveEntries(i)<<endl;
...
ratio_hPPb_JetIDcut_pt_fakevsInc->GetXaxis()->SetMoreLogLabels();
...
ratio_hPPb_pt_fakevsInc->GetXaxis()->SetMoreLogLabels();
