...
else if(ilist== 6)  {    JetID = "chargedMax/p_{T}"; JetIDcut[0]=0; JetIDcut[1]=0.5;}
...
else if(ilist== 12) {    JetID = "(#Sigma h^{#pm}+#Sigma #gamma +#Sigma h^{0}+#Sigma #mu+#Sigma e)/p_{T}^{jet}";JetIDcut[0]=0; JetIDcut[1]=2.00;}
else if(ilist== 13) {    JetID = "(#Sigma h^{#pm}+#Sigma #gamma +#Sigma h^{0}+#Sigma #mu+#Sigma e)/p_{T}^{raw}"; JetIDcut[0]=0;JetIDcut[1]=1.13;}
...
TH1D* rehisto_hPP_JetIDcut_pt=(TH1D*)myRebin(hPP_JetIDcut_pt,Nbin_pt,binbound_pt);
TH1D* rehisto_hPP_JetIDcut_pt_real=(TH1D*)myRebin(hPP_JetIDcut_pt_real,Nbin_pt,binbound_pt);
TH1D* rehisto_hPP_JetIDcut_pt_fake=(TH1D*)myRebin(hPP_JetIDcut_pt_fake,Nbin_pt,binbound_pt);
TH1D* rehisto_hPP_JetID=(TH1D*)myRebin(hPP_JetID,Nbin_JetID,binbound_JetID);
TH1D* rehisto_hPP_JetID_real=(TH1D*)myRebin(hPP_JetID_real,Nbin_JetID,binbound_JetID);
TH1D* rehisto_hPP_JetID_fake=(TH1D*)myRebin(hPP_JetID_fake,Nbin_JetID,binbound_JetID);
...
rehisto_hPP_pt->SetFillColor(1);
rehisto_hPP_pt->SetMarkerColor(1);
rehisto_hPP_pt->SetLineColor(1);
rehisto_hPP_pt->SetMarkerStyle(24);
rehisto_hPP_pt->SetMarkerSize(1.5);
rehisto_hPP_pt->SetTitle(Form("PYTHIA %.2f--%.2f %s",xrange_JetID[0],xrange_JetID[1], Unit.Data()));
rehisto_hPP_pt->GetYaxis()->SetTitle("Probability");
rehisto_hPP_JetIDcut_pt->SetTitle(Form("PYTHIA %.2f--%.2f %s",xrange_JetIDcut[0],xrange_JetIDcut[1], Unit.Data()));
rehisto_hPP_JetIDcut_pt->GetYaxis()->SetTitle("Probability");

rehisto_hPPb_pt->SetTitle(Form("PYTHIA+HIJING %.2f--%.2f %s",xrange_JetID[0],xrange_JetID[1], Unit.Data()));
...
rehisto_hPPb_JetIDcut_pt->SetTitle(Form("PYTHIA+HIJING %.2f--%.2f %s",xrange_JetIDcut[0],xrange_JetIDcut[1], Unit.Data()));
...
T4.SetTextSize(0.03);
T4.SetTextFont(42);
T4.DrawLatex(0.65,0.32,Form("p_{T}: %.f-%.f (GeV/c)",xrange_pt[0],xrange_pt[1]));
...
TLegend *leg2;
if(ilist==7)
leg2=new TLegend(0.7,0.32,0.9,0.45);
else
leg2=new TLegend(0.7,0.72,0.9,0.85);
...
T7.SetTextSize(0.03);
T7.SetTextFont(42);
T7.DrawLatex(0.65,0.32,Form("p_{T}: %.f-%.f (GeV/c)",xrange_pt[0],xrange_pt[1]));
...
T2->SetTextFont(42);
T2->Draw("same");
c2->Print(Form("pic/%s/jetid_%.f-%.f.png",JetIDName.Data(),xrange_pt[0],xrange_pt[1]));

//---------------------------------------PP&PPb JetID f/i&r/i ratio before cut ------------------------------------------ 
TCanvas* c3 = new TCanvas("c3"," ",1200,600);
makeMultiPanelCanvas(c3,2,1,0,0,0.15,0.1,0.03);
c3->cd(1)->SetLogy();
TH1D* ratio_hPP_JetID_fake=(TH1D*)rehisto_hPP_JetID_fake->Clone("ratio_hPP_JetID_fake");
...
TH1D* ratio_hPP_JetID_real=(TH1D*)rehisto_hPP_JetID_real->Clone("ratio_hPP_JetID_real");
ratio_hPP_JetID_real->Divide(rehisto_hPP_JetID);
ratio_hPP_JetID_real->Draw("same");
T1->Draw("same");
T7.DrawLatex(0.78,0.82,Form("p_{T}: %.f-%.f (GeV/c)",xrange_pt[0],xrange_pt[1]));
c3->cd(2)->SetLogy();
TH1D* ratio_hPPb_JetID_fake=(TH1D*)rehisto_hPPb_JetID_fake->Clone("ratio_hPPb_JetID_fake");
...
TH1D* ratio_hPPb_JetID_real=(TH1D*)rehisto_hPPb_JetID_real->Clone("ratio_hPPb_JetID_real");
ratio_hPPb_JetID_real->Divide(rehisto_hPPb_JetID);
ratio_hPPb_JetID_real->Draw("same");
TLegend *leg3=new TLegend(0.65,0.70,0.85,0.80);
...
leg3->Draw("same");
T2->Draw("same");
c3->Print(Form("pic/%s/ratio_jetid_%.f-%.f.png",JetIDName.Data(),xrange_pt[0],xrange_pt[1]));
/*
...
ratio_hPP_JetIDcut_pt_fakevsInc->SetMinimum(1e-7);
...
ratio_hPP_pt_fakevsInc->SetMinimum(1e-7);
...
ratio_hPPb_JetIDcut_pt_fakevsInc->SetMinimum(1e-7);
...
ratio_hPPb_pt_fakevsInc->SetMinimum(1e-7);
...
TLegend *leg7=new TLegend(0.65,0.15,0.85,0.25);
