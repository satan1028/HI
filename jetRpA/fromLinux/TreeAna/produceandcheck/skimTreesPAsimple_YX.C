...
if(!isMC){
 ...
 int dupRuns[6] = {181912,181913,181938,181950,181985,182124};
  ...
  Int_t photonN[100];
  Int_t neutralN[100];
  Float_t chargedMax[100];
  Float_t photonMax[100];
  Float_t neutralMax[100];
  Float_t chargedSum[100];
  Float_t photonSum[100];
  Float_t neutralSum[100];
  Float_t muSum[100];
  Float_t eSum[100];
  ...
  if(isMC) datatype = "MC";
   ...
   fout=new TFile(Form("%s%sskimfile%d_%d.root",datatype.Data(), coll.Data(), startfile,endfile),"recreate");
...
if(isMC)  nt->Branch("subid",subid,"subid[nref]/I");
  nt->Branch("rawpt",rawpt,"rawpt[nref]/F");
if(isMC)    nt->Branch("refpt",refpt,"refpt[nref]/F");
    if(isMC)    nt->Branch("refeta",refeta,"refeta[nref]/F");
