...
const double pPbLumi = 34.7 ; //excluded the old alignment run for pPb
void DrawIterationData(){
...
hFrame->GetYaxis()->SetRangeUser(0.9,1.45);
hFrame->GetYaxis()->SetTitle("Ratio (Unfolded/Normial)");
...
hReco_cent_Cl[i][j]->Divide(hReco_cent0[i]);
	...
	for(int ibin=1;ibin<=IterMax[i]->GetNbinsX();ibin++){
	if(IterMax[i]->GetBinCenter(ibin)>50 && IterMax[i]->GetBinCenter(ibin)<600){
	fstr[i]<<IterMax[i]->GetBinCenter(ibin)<<'\t';
//	fstr[i]<<IterMax[i]->GetXaxis()->GetBinLowEdge(ibin)<<"to"<<IterMax[i]->GetXaxis()->GetBinUpEdge(ibin)<<'\t';
	fstr[i]<<TMath::Abs(IterMax[i]->GetBinContent(ibin)-1)*100<<endl;
}
}
}	

TLegend* leg1=new TLegend(0.18,0.60,0.38,0.88);
...
for(int i=0;i<Neta-1;i++) if(i!=3) IterMax[i]->DrawCopy("same");
//hUnfoldedJeCsys_cent0_Cl->DrawCopy("same");
//leg2->AddEntry(hUnfoldedJeCsys_cent0_Cl,"UnfoldedJeCsys","lp");
leg1->Draw("same");

drawCMS(0.5,0.88,pPbLumi);

TLatex *com3 = new TLatex(0.45,0.17,"Anti-k_{T} Particle Flow PU Jets: R=0.3");
com3->SetTextFont(63);
com3->SetTextSize(17);
com3->SetNDC();
com3->Draw();
